...
1. A theoretical overview of ssSeq. Jump to [Theoretical Overview](#Theoretical-Overview)
2. Information and resources needed to prepare ssSeq libraries for next-gen sequencing. Jump to [Biological Protocols](#Biological-Protocols).
3. Install ssSeq software needed for data analysis post sequencing. Jump to [Installation](#Installation)
4. Run ssSeq software through either a graphic user interface (GUI) or command line. Jump to [Program Arguments](#Program-Arguments), [Working with the GUI](#Working-with-the-GUI), and/or [Working through Command Line](#Working-through-Command-Line).
5. Understand the output generated by the ssSeq software. Jump to [Understanding ssSeq Output](#Understanding-ssSeq-Output).
...
# Example Data
The folder [InstallationConfirmationData](./InstallationConfirmationData) contains an example reference sequence file and example fastq.gz files. These files can be used for confirming installation or just playing around with ssSeq. 
